...
const computeCaptureZone = (
    ...
    if (!isWithinJiNotationLevel(jiNotationSymbolClass.introducingJiNotationLevel, jiNotationLevel)) {
        throw new Error(`JI Notation symbol class ${formatSymbolClass(jiNotationSymbolClass.id, {
            ...ioSettings,
            forTable: false,
        })} is not present at the ${formatJiNotationLevel(jiNotationLevel)} JI notation level; it is not introduced until the ${formatJiNotationLevel(jiNotationSymbolClass.introducingJiNotationLevel)} JI notation level.`)
    }
