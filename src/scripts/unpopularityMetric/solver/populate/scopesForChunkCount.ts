...
const populateScopesForChunkCount = async (chunkCount: Count<Chunk>) => {
    if (debug.all) { console.log(`computing scopes for chunk count ${chunkCount}: phase 1 of ${chunkCount} (please give lead time for combinations calculation)`.cyan) }
    ...
    while (chunkCountForSubmetrics > 1) {
        ...
        if (debug.all) { console.log(`computing scopes for chunk count ${chunkCount}: phase ${chunkCountForParameters + 1} of ${chunkCount} (please give lead time for combinations calculation)`.cyan) }
        ...
        if (debug.all) { console.log(`combinations calculated: ${submetricChunkCombinations.length} submetric combinations and ${parameterChunkCombinations.length} parameter combinations`.cyan) }
        for (const submetricChunkCombination of submetricChunkCombinations) {
            ...
            if (debug.all) { console.log(`finished for one submetric chunk combination within this phase ${debugSearchedAndPopulated()}`.cyan) }
        }
