...
const populateScopesForChunkCountPhase = async (chunkCount: Count<Chunk>, chunkCountForSubmetrics: Count<SubmetricChunk>) => {
    ...
    saveDebugMessage(`computing scopes for chunk count ${chunkCount}: phase ${1 + chunkCount - chunkCountForSubmetrics}/${chunkCount}`, DebugTarget.POPULATION)
    ...
    if (memoizedSubmetricChunkCombinations[ chunkCountForSubmetrics ]) {
        ...
        saveDebugMessage(`used memoized submetric combinations (with repetitions) for chunk count ${chunkCountForSubmetrics}`, DebugTarget.POPULATION)
    } else {
    ...
    if (memoizedParameterChunkCombinations[ chunkCountForParameters ]) {
        ...
        saveDebugMessage(`used memoized parameter combinations (with repetitions) for chunk count ${chunkCountForParameters}`, DebugTarget.POPULATION)
    } else {
    ...
    saveDebugMessage(`finished phase ${1 + chunkCount - chunkCountForSubmetrics}/${chunkCount} of scope population for chunk count ${chunkCount} ${presentSearchedAndPopulated()}`, DebugTarget.POPULATION)
}
