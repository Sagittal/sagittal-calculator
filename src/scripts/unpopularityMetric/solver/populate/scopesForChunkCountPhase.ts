...
const populateScopesForChunkCountPhase = async (chunkCount: Count<Chunk>, chunkCountForSubmetrics: Count<SubmetricChunk>) => {
    ...
    saveDebugMessage(`computing scopes for chunk count ${chunkCount}: phase ${1 + chunkCount - chunkCountForSubmetrics}/${chunkCount}`, DebugTarget.POPULATE)
    ...
    if (memoizedSubmetricChunkCombinations[ chunkCountForSubmetrics ]) {
        ...
        saveDebugMessage(`used memoized submetric combinations (with repetitions) for chunk count ${chunkCountForSubmetrics}`, DebugTarget.POPULATE)
    } else {
    ...
    if (memoizedParameterChunkCombinations[ chunkCountForParameters ]) {
        ...
        saveDebugMessage(`used memoized parameter combinations (with repetitions) for chunk count ${chunkCountForParameters}`, DebugTarget.POPULATE)
    } else {
    ...
    saveDebugMessage(`we find ${exampleDistributions.length} distributions of ${parameterChunkCombinations[ 0 ].length} parameter chunks across ${submetricChunkCombinations[ 0 ].length} bins (assignments to each of a combination of submetrics, plus an extra bin for parameters which will get applied to every submetric), which is how many more scopes should be contributed per each of the ${parameterChunkCombinations.length} parameter chunk combinations in this phase, and that times the ${submetricChunkCombinations.length} submetric chunk combinations in this phase, so expect ${exampleDistributions.length} * ${parameterChunkCombinations.length} * ${submetricChunkCombinations.length} = ${exampleDistributions.length * parameterChunkCombinations.length * submetricChunkCombinations.length} new scopes from this phase, so we should end with a total of ${(populatedsForChunkCount[ chunkCount ] || 0) + exampleDistributions.length * parameterChunkCombinations.length * submetricChunkCombinations.length}`, DebugTarget.POPULATE)
    ...
    saveDebugMessage(`finished phase ${1 + chunkCount - chunkCountForSubmetrics}/${chunkCount} of scope population for chunk count ${chunkCount} ${presentSearchedAndPopulated()}`, DebugTarget.POPULATE)
}
