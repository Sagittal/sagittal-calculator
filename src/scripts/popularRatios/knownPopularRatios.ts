import { computeMonzoFromRatio, Formatted, parseRatio, rank, Ranked, RankStrategy, Ratio } from "../../general"
...
const computeKnownPopularRatios = (): Array<Ranked<PopularRatio>> => {
    const knownPopularRatios = [
        ...
        "209/25",
        "235/1",
    ] as Array<Formatted<Ratio>>

    const popularRatios = knownPopularRatios
        .map(parseRatio)
        .map(computeMonzoFromRatio)
        .map(monzo => computePopularRatio({ monzo, n2d3p9: computeN2D3P9(monzo) }))

    return rank(popularRatios, { by: "n2d3p9", strategy: RankStrategy.FRACTIONAL })
}

export {
    computeKnownPopularRatios,
}
