...
import {N2D3P9, TINA, Tina, TINA_CENTS} from "../../../sagittal"
...
const TINA_COMMAS_PLUS_MINUS_RANGE: Tina = 0.25 as Tina

const TINA_COMMAS_LOWER_BOUND = computePitchFromCents(
    (TINAS_TO_CHECK[0] - TINA_COMMAS_PLUS_MINUS_RANGE) * TINA_CENTS as Cents, // TODO: computeCentsFromTinas
) as Min<Scamon<{rational: false}>>
const TINA_COMMAS_UPPER_BOUND = computePitchFromCents(
    (TINAS_TO_CHECK[TINAS_TO_CHECK.length - 1] + TINA_COMMAS_PLUS_MINUS_RANGE) * TINA_CENTS as Cents,
) as Max<Scamon<{rational: false}>>

const MAX_TINA_SIZES: Cents[] =
    TINAS_TO_CHECK.map((tina: Tina): Cents => TINA_CENTS * (tina + TINA_COMMAS_PLUS_MINUS_RANGE) as Cents)
