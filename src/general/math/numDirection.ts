...
// TODO: it's time to create a math/num directory and move everything in rational deeper nested into it.
...
const computeSuperNum = <T extends NumTypeParameters, U extends Num<T>>(
    num: U,
): Exclude<U, Num> & Num<Omit<T, "_DirectionBrand"> & { direction: Direction.SUPER }> => {
    const isSubNum = computeIsSubNum(num)

    let superNum: Exclude<U, Num> & Num<T & { direction: Direction.SUPER }> =
        {} as Exclude<U, Num> & Num<T & { direction: Direction.SUPER }>
    if (isSubNum) {
        const { monzo, ratio, decimal } = num
        if (!isUndefined(ratio)) {
            superNum.ratio = invertRatio(ratio) as Ratio<T>
        }
        if (!isUndefined(monzo)) {
            superNum.monzo = invertMonzo(monzo) as Monzo<T>
        }
        if (!isUndefined(decimal)) {
            superNum.decimal = reciprocal(decimal) as Decimal<T>
        }
    } else {
        superNum = deepClone(
            num as Exclude<U, Num> & Num<T & { direction: Direction.SUPER }>,
        )
    }
