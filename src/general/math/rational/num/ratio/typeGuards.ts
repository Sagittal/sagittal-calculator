...
//  We have the ability to do that already from computeRationalRatioFromRationalDecimal. just call it on both parts.
...
const isRationalRatio = <T extends NumTypeParameters>(
    candidateRationalRatio: Ratio<T>,
): candidateRationalRatio is RationalRatio<T> => {
...
const isIntegerRatio = <T extends NumTypeParameters>(
    candidateIntegerRatio: Ratio<T>,
): candidateIntegerRatio is RationalRatio<T & { integer: true }> => {
