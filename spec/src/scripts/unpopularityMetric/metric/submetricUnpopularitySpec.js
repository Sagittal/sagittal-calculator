...
const {ADJUSTMENT, SUBMETRIC_OPERATION} = require("../../../../../src/scripts/unpopularityMetric/parameters/constants")

describe("computeSubmetricUnpopularity", () => {
    let adjustments
    let submetricType
    ...
    describe("default case: submetric type sums primes, not using prime index, using repetition (adjustments tested here)", () => {
        it("sums the absolute values of the prime factors in the 5-rough monzo", () => {
            const result = computeSubmetricUnpopularity(fiveRoughNumberMonzo, adjustments, submetricType)
        ...
        it("works when a is used as a base, not a power", () => {
            const a = 0.56
            adjustments[ADJUSTMENT.A] = a
            adjustments[ADJUSTMENT.A_IS_BASE_NOT_POWER] = 1
    ...
    describe("when the submetric type sums primes (not counts primes)", () => {
        beforeEach(() => {
            submetricType.operation = SUBMETRIC_OPERATION.SUM
        })
    ...
    describe("when the submetric type counts primes (not sums primes)", () => {
        beforeEach(() => {
            submetricType.operation = SUBMETRIC_OPERATION.COUNT
        })
    ...
    it("works for an empty monzo", () => {
        const result = computeSubmetricUnpopularity([], adjustments, submetricType)
