...
describe("solve best metric", () => {
    it("finds the best metric for the given range of chunk count", () => {
        ...
        expect(result).toEqual([
            ...
            `parameter combinations (with repetitions) computed: 1; formula is ((0+15-1)!)/((0!)((15-1)!)) where 15 is the total of possible existing chunks and 0 is the count we are choosing at a time`,
            ...
            ``,
            ``,
            `AND THE TIMED OUT METRIC COUNTS PER CHUNK COUNT WERE []`,
        ])
    })
})
