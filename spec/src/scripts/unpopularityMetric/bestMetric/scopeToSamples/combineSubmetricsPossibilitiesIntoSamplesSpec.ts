...
describe("combineSubmetricsPossibilitiesIntoSamples", () => {
    it("takes the list of possible values for each submetric individually, and returns a list of every possible combination of them, along with its corresponding sample point, which is then all together called a Sample; it also takes the zeroth submetric scope (the all bins one) and spreads it across every submetric", () => {
        ...
        const dynamicParameters: DynamicParameter[] = [
            ...
            // submetric two had 4 possibilities, combinations of two dynamic parameters which each have two possibilities
            // (and yes it features a dynamic parameter which is the same parameter as one in a different submetric -- y is shared with submetric one -- but that's perfectly okay)
            {
        ...
        const actual: Sample[] = combineSubmetricsPossibilitiesIntoSamples({ submetricsPossibilities, dynamicParameters })

        // samplePoint: [ submetric zero's one dynamic parameter, submetric one's one dynamic parameter, submetric two's first dynamic parameter, submetric two's second dynamic parameter ]
        const expected = [
            // in this section, submetric zero (all bins) is at its possibility one of two
            {
                submetrics: [
            ...
            // in this section, submetric zero (all bins) is at its possibility two of two, but otherwise a copy of the above
            {
                submetrics: [
