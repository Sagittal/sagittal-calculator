...
import { computeMetricName } from "../../../../../../../src/scripts/unpopularityMetric/solver/search/bestMetric/metricName"

describe("computeMetricName", () => {
    it("makes a string out of the parameters (ignoring values) in each submetric", () => {
        const submetrics = [
            {
                [ Parameter.A_AS_BASE ]: 2.1,
                [ Parameter.SUM ]: true,
            },
            {
                [ Parameter.MAX ]: true,
        ...
        expect(result).toBe(`{aAsBase,sum},{max,withoutRepetition}`)
    })
    ...
    it("sorts the parameters within each name (so that metrics which are the same get consolidated)", () => {
        ...
        expect(result).toBe(`{aAsBase,sum},{aAsExponent,max,withoutRepetition}`)
    })
})
