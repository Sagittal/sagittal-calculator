...
describe("ensureOneBestPossibleEventPerJiNotationLevel", (): void => {
    it(
        `throws an error if a bound history consolidation has more than one event in a single JI notation level 
        which is considered to be the one which is a member of the best possible bound history`,
        (): void => {
    ...
    it(
        `throws an error if a bound history consolidation has, at a particular JI notation level, 
        no event which is identified as being the member of the best possible bound history`,
        (): void => {
            const boundHistoryConsolidation: BoundHistoryConsolidation = {
                [ JiNotationLevel.MEDIUM ]: [
                    { ...boundEventConsolidationFixture, isBestPossibleBoundHistoryMember: false },
    ...
    it(
        `does not throw an error if a bound history consolidation does not have more than one best possible event per JI notation level`,
        (): void => {
            const boundHistoryConsolidation: BoundHistoryConsolidation = {
                [ JiNotationLevel.MEDIUM ]: [
                    { ...boundEventConsolidationFixture, isBestPossibleBoundHistoryMember: true },
