...
import {usefulnessMetricLfcScriptGroupSettings} from "../../../../src/scripts/usefulnessMetricLfc/globals"
import {USEFULNESS_METRIC_FAMILIES_WITH_PARAMETERS} from "../../../../src/scripts/usefulnessMetricLfc/metrics"
import {computeUsefulnessMetricScoreForCommaZone} from "../../../../src/scripts/usefulnessMetricLfc/metricScoreForZone"
import {
...
describe("computeUsefulnessMetricScoreForCommaZone", (): void => {
    ...
    describe("when in sos mode", (): void => {
        beforeEach((): void => {
            usefulnessMetricLfcScriptGroupSettings.sosMode = true
        })

        it("returns 0 when a comma is the most useful comma in its zone", (): void => {
            const actual = computeUsefulnessMetricScoreForCommaZone(
                [CommaClassId._1_455_n, COMMAS_FOR_1_455_n],
                usefulnessMetric,
                usefulnessParameterSet,
            )

            const expected = 0 as UsefulnessMetricScoreForZone
        ...
        it("returns a squared distance between the actual comma's usefulness and the most useful comma when a comma is not the most useful comma in its zone", (): void => {
            ...
            const expected = 1.050655 as UsefulnessMetricScoreForZone
            expect(actual).toBeCloseTo(expected)
        })
    })
    ...
    describe("when in boolean mode", (): void => {
        beforeEach((): void => {
            usefulnessMetricLfcScriptGroupSettings.sosMode = false
        })
        ...
        it("returns 1 point (bad) when a comma is not the most useful comma in its zone", (): void => {
            ...
            // 1/8575's usefulness is 7.866050, but 17/19k's usefulness is 6.841035; (7.866050 - 6.841035)^2 = 1.050655
            const expected = 1 as UsefulnessMetricScoreForZone
            expect(actual).toBe(expected)
        })
    })
})
